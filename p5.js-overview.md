...
You can use any editing environment you like, in addition, a [p5.js editor](http://p5js.org/download/#editor) is in development, you can try out the alpha version now. The p5.js editor handles setting up projects and runs a local server for you. You can usually run p5.js sketches without a server, but it is required if you are doing certain things like loading external files. Here are instructions for [setting up a local server](https://github.com/processing/p5.js/wiki/Local-server).
